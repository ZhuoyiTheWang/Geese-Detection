import {
  ...
  export default function ImageTable({ entries, counts, onEntryClick }) {
    ...
    const rowsToDisplay = entries.slice(
      page * rowsPerPage,
              ...
              {rowsToDisplay.map((entry, index) => (
                <TableRow key={index}>
                  <TableCell
                    ...
                    onClick={() =>
                      onEntryClick && onEntryClick(entry.fileURL)
                    } // Use onEntryClick for image upload or display
                  >
                    {entry.name}
                  </TableCell>
                  <TableCell>{entry.park || "Unknown"}</TableCell>
                  {/* Display the corresponding count */}
                  <TableCell>{counts ? counts[index] ?? 0 : entry.count ?? 0}</TableCell>
                </TableRow>
              ))}
              {emptyRows > 0 &&
