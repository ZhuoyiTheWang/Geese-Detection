#Need to convert from xml file to file format expected by yolo
import xmltodict
import os
import glob
...
for xml_file in glob.glob("Data/*.xml"): #for each xml file in dataset downloaded
    ...
    with open(f'datasets/labels/train/{xml_file[-8:-4]}.txt','w') as f: #create label file
...
for image_file in glob.glob("Data/*.jpg"): #for each image file in dataset download
    ...
    destination_path = f'datasets/images/train/{image_file[-8:]}'
